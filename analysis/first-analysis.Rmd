...
The %GC content after alignment looks a bit dodgy. Is this because of inproper ribosomal RNA removal? 
...
read_excel("data/rseqc_readDist_summary.xlsx") %>% 
  ...
  left_join(meta) %>% 
  ggplot(aes(y = geo_accession, x = `Tags/Kb`, fill = genomicFeature)) +
  geom_col() +
...
Including the -M flag meant that there was no more multi-mapping reads. However, now the # reads which were unassigned due to mapping quality was reduced. I then removed the -Q flag to see, and then they were simply unassigned due to no features. 
...
fcmm <- read.delim("data/04_featureCounts/featureCounts.all.multimapping.mappingqual10.txt.summary")
```
